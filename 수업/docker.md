...
- **개발 환경 일관성 보장**: 로컬에서 실행되는 코드가 운영 환경에서도 동일하게 작동
- **애플리케이션의 빠른 배포**: 가볍고 독립적인 단위로 패키징
- **효율적인 자원 사용**: 가상머신보다 훨씬 빠르고 가볍게 실행
...
| 구성 요소     | 설명 |
|--------------|------|
| **이미지(Image)** | 컨테이너를 만들기 위한 템플릿. 여러 레이어로 구성 |
| **컨테이너(Container)** | 이미지로부터 실행된 인스턴스. 실행 중 상태 포함 |
| **Dockerfile** | 이미지를 정의하는 스크립트 파일 |
| **Docker CLI / API** | 도커를 조작하는 명령줄 도구 또는 API |
| **Docker Engine** | 실제 컨테이너를 관리하고 실행하는 핵심 데몬 |
| **Docker Hub** | 공개 이미지 저장소(레지스트리). GitHub처럼 역할 |

---

## ✅ 3. Docker vs 가상머신(VM)

| 항목 | Docker(컨테이너) | 가상머신(VM) |
|------|------------------|---------------|
| 성능 | 매우 빠름 | 느림 |
| 자원 사용 | 작음 (OS 공유) | 큼 (OS 별도) |
| 시작 속도 | 수 초 | 수 분 |
| 격리 수준 | 낮음 (커널 공유) | 높음 (완전한 OS) |
| 사용 목적 | 앱 배포, 마이크로서비스 | 강력한 보안, 완전한 격리 필요 시 |
