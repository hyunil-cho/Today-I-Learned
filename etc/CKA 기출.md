...
7. Deployment를 NodePort로 노출

```

이미 존재하는 Deployment를, NodePort로 노출하면 됩니다. 단, 만약, kubectl expose 명령어를 통해 노출하고자 할 경우에는, 반드시 서비스 이름을 체크해야 합니다. 단순 expose 명령어로 사용하게 되면, 문제에서 요구하는 서비스명과 달라 감점이 될 수 있습니다. 그렇기 때문에, --name 옵션을 통해, 문제에서 주어진 서비스의 이름과 동일하게 맞추는 게 중요!@
